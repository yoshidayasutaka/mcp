...
class CostAnalysisHelper:
            ...
            if price_section_match:
                ...
                for pattern in price_patterns:
                    ...
                    for match in matches:
                        if len(match) == 2:
                            if (
                                pattern == price_patterns[0]
                            ):  # First pattern has price first
                                price, unit = match
                                ...
                                for term_type, term_values in terms.items():
                                    for _, price_dimensions in term_values.items():
                                        for _, dimension in price_dimensions.items():
                                            if (
                                                "pricePerUnit" in dimension
                                                and "unit" in dimension
                                            ):
                                                unit = dimension["unit"]
                                                price = dimension.get(
                                                    "pricePerUnit", {}
    ...
    def generate_cost_table(pricing_structure: Dict) -> Dict:
        ...
        free_tier_info = pricing_structure.get(
            "free_tier", "No free tier information available"
        )
        ...
        usage_cost_table += (
            f"| {service_name} | {low_cost} | {med_cost} | {high_cost} |\n"
        )
